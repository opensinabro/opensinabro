...
파서·렌더러 검증용 입력 데이터입니다. 워크스페이스의 여러 크레이트가 공유하며, 성격이 다른 두
묶음으로 나뉩니다.

| 디렉토리 | 내용 |
|---|---|
| [`documents/`](documents) | 실제 나무위키 문서 원문 12건. 골든 스냅샷 대상. |
| [`corpus/`](corpus) | 문법 하나만 담은 회귀 코퍼스 287건. |
...
- 각 파일명은 문서 제목의 특수문자를 `_`로 치환한 것입니다 (`documents/manifest.json` 참고).

## `corpus/` — 문법 회귀 코퍼스

실제 문서가 아니므로 아래 골든 대상에서는 제외됩니다. 케이스가 원문·의미론·렌더링을 한 파일에
담아 자기완결적이라 별도 골든 디렉토리가 없습니다. **원문 일부는 알파위키 문법 도움말의 예제를
그대로 옮긴 것입니다** — 출처와 라이선스는 `corpus/README.md` 참고.

## 골든 스냅샷

실제 문서(`documents/*.namu`)의 골든 스냅샷은 계층별로 나뉩니다.
