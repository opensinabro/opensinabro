...
| 파일 업로드 | P2 | `/upload` — 파일명·라이선스 선택·분류 지정 필수. `[[파일:이름]]` 첨부. 문서 보기는 `/w/파일:이름`, 바이너리 서빙은 `/file/<파일명>` |
...
| 기능 | 우선순위 | 내용 |
|---|---|---|
| 토론 스레드 | P2 | `/discuss/문서`(목록·생성), `/thread/<uuid>`(댓글 #번호 단위) |
| 스레드 상태 | P2 | normal / pause / close. 주제 변경, 다른 문서로 이동 |
| 댓글 관리 | P3 | 블라인드·복구, 관리자 `[ADMIN]` 표시 발언 |
| 편집요청 | P2 | 편집 권한이 없을 때 변경안 제출 → 권한자가 Accept/Close, 요청자 수정 가능. 개별 요청은 `/edit-request/<uuid>` |
...
- **자체 호스팅 1순위**: `docker compose up` 한 번으로 곧장 돌아가야 한다. 필요한 것은
  PostgreSQL뿐이고, 검색은 서버에 내장(tantivy)해 별도 검색엔진·캐시 서버를 두지 않는다.
  openNAMU가 채운 수요(소규모 개인·커뮤니티 위키)와 같은 자리이되, the seed 동작 충실도는
  이 프로젝트의 파서·렌더러 수준을 따른다.
...
- **보안 표준**: 세션 쿠키는 `HttpOnly`·`SameSite=Lax`·(HTTPS에서)`Secure`. 상태를
  바꾸는 모든 POST 폼에 CSRF 토큰(동기화 토큰 패턴). 비밀번호는 argon2(OWASP 권장),
  로그인·가입·편집에 요청 제한(tower 미들웨어). 위키 입력이 원시로 나가는 통로는
  렌더러가 이미 화이트리스트 처리(`#!html`·`#!wiki` style).
