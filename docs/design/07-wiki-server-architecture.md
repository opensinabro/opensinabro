...
|---|---|---|---|
| `wiki-account` | 행위 주체 | actor·user·user_credential·user_email·user_preference·notification·star | 가입·인증(비밀번호·TOTP)·IP 사용자·알림·북마크. `ActorIdentifier` 등 참조 타입의 원천 |
...
- **의존은 참조 방향**: 리비전이 actor를 참조하므로 document → account, 토론이 문서에
  ...
  document·account. 순환 없음. 마이그레이션은 크레이트별 소유 파일을 바이너리가 모은다.
