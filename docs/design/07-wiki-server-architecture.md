...
|---|---|---|---|
| `wiki-account` | 행위 주체 | actor·wiki_user·user_credential·user_email·user_preference·notification | 가입·인증(비밀번호·TOTP)·IP 사용자·알림. `ActorIdentifier` 등 참조 타입의 원천. **다른 크레이트를 참조하지 않는 의존 그래프의 뿌리** |
| `wiki-document` | 문서와 그 역사 | document·revision·document_reference·render_cache·file_content·file_revision·star | 리비전 채번·이동·삭제·diff·3-way 병합·역링크·파일·렌더 캐시·문서 구독. `WikiContext` 구현과 렌더 파이프라인 연결도 여기 — 링크 존재·include 원문이 곧 이 크레이트의 데이터다 |
